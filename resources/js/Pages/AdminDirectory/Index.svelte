<script>
    ...
    function submitSearch(e) {
        e.preventDefault();
        router.get(route('admin-directory.index'), { search }, { preserveState: true, replace: true });
    }
...
<AppLayout title="Danh sách quản trị viên">
    <div class="mx-auto max-w-6xl py-6">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Quản trị viên</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Danh sách tài khoản quản trị và quyền truy cập tương ứng.</p>
            </div>
            <Link href={route('admin-directory.create')} class="flex items-center gap-2 rounded-lg bg-accent-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-accent-700">
        ...
        <form onsubmit={submitSearch} class="mb-4">
            <div class="relative max-w-sm">
                ...
                <input type="text" bind:value={search} placeholder="Tìm theo tên, username, email..." class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 pl-10 text-sm text-gray-900 focus:border-accent-500 focus:ring-accent-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
            </div>
        </form>
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-800">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900/40">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Họ và tên</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Username</th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                    {#each admins.data as admin}
                        <tr class="group transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{admin.name}</td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{admin.username}</td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{admin.email}</td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{admin.role ?? '—'}</td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm">
                                <span class="rounded-full px-2.5 py-1 text-xs font-medium {badgeClass(admin.status)}">{badgeLabels[admin.status] ?? admin.status}</span>
                            </td>
                    ...
                    {:else}
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-sm text-gray-500 dark:text-gray-400">Chưa có quản trị viên nào.</td>
                        </tr>
                    {/each}
                </tbody>
            </table>
        ...
        {#if admins.links && admins.links.length > 3}
