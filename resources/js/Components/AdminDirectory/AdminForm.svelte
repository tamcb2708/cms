<script>
    ...
    let { form, roles = [], mode = 'create', adminId = null } = $props();
    ...
    onMount(() => {
        ...
        pickers = [
            flatpickr(validFromInput, {
                ...opts,
                defaultDate: $form.valid_from || undefined,
                onChange: ([date]) => ($form.valid_from = date ? formatDate(date) : ''),
            }),
            flatpickr(validUntilInput, {
                ...opts,
                defaultDate: $form.valid_until || undefined,
                onChange: ([date]) => ($form.valid_until = date ? formatDate(date) : ''),
            }),
        ];
    });
    ...
    function submit(e) {
        e.preventDefault();
        if (mode === 'edit') {
            $form.put(route('admin-directory.update', adminId));
        } else {
                ...
                <div class="grid grid-cols-1 gap-6 border-t border-gray-100 pt-4 dark:border-gray-700 md:grid-cols-2">
                    <div class="space-y-2">
                        <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Mật khẩu {#if mode === 'create'}<span class="text-red-500">*</span>{/if}
                        </label>
                        ...
                        {#if mode === 'edit'}<p class="text-xs text-gray-500">Để trống nếu không muốn đổi mật khẩu.</p>{/if}
                        {#if $form.errors.password}<p class="mt-1 text-xs text-red-500">{$form.errors.password}</p>{/if}
                    </div>
                    <div class="space-y-2">
                        <label for="password_confirmation" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Xác nhận mật khẩu {#if mode === 'create'}<span class="text-red-500">*</span>{/if}</label>
